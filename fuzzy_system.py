...
class FuzzySystem:
    ...
    def create_input_variables(self):
        ...
        for question in self.question_repository.select_all():
            ...
            self.input_variables[question.id]['no'] = fuzz.trimf(self.input_variables[question.id].universe, [0, 0, one_half])
            self.input_variables[question.id]['probably no'] = fuzz.trimf(self.input_variables[question.id].universe, [0, one_quarts, one_half])
            self.input_variables[question.id]['maybe'] = fuzz.trimf(self.input_variables[question.id].universe, [0, one_half, self.INPUT_RANGE])
            self.input_variables[question.id]['probably yes'] = fuzz.trimf(self.input_variables[question.id].universe, [one_half, three_quarts, self.INPUT_RANGE])
            self.input_variables[question.id]['yes'] = fuzz.trimf(self.input_variables[question.id].universe, [one_half, self.INPUT_RANGE, self.INPUT_RANGE])
    ...
    def create_output_variables(self):
        ...
        for person in self.person_repository.select_all():
            ...
            self.output_variables[person.id]['low'] = fuzz.trimf(self.output_variables[person.id].universe, [0, 0, one_half])
            self.output_variables[person.id]['medium low'] = fuzz.trimf(self.output_variables[person.id].universe, [0, one_quarts, one_half])
            self.output_variables[person.id]['medium'] = fuzz.trimf(self.output_variables[person.id].universe, [0, one_half, self.OUTPUT_RANGE])
            self.output_variables[person.id]['medium high'] = fuzz.trimf(self.output_variables[person.id].universe, [one_half, three_quarts, self.OUTPUT_RANGE])
            self.output_variables[person.id]['high'] = fuzz.trimf(self.output_variables[person.id].universe, [one_half, self.OUTPUT_RANGE, self.OUTPUT_RANGE])

    def create_rules(self):
        for person in self.person_repository.select_all():
            for question in self.question_repository.select_all():
                for response in self.response_repository.select_by_data(person.id, question.id):

                    outputs = ('low', 'medium low', 'medium', 'medium high', 'high')
                    for i, resp in enumerate(('no', 'probably no', 'maybe', 'probably yes', 'yes')):
                        antecedent = self.input_variables[question.id][resp]
                        consequent = self.output_variables[person.id][outputs[4 - abs(i - response.response * 4)]]
                        
                        self.rule.append(ctrl.Rule(antecedent, consequent))
